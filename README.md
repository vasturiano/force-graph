...
| <b>nodeCanvasObjectMode</b>([<i>str</i>]) | Custom node drawing function mode. Use in combination with `nodeCanvasObject` to specify how to customize nodes painting. Possible values are: <ul><li>`replace`: node are rendered using just `nodeCanvasObject`.</li><li>`before`: nodes are rendered by invoking `nodeCanvasObject` and then proceeding with the default node painting.</li><li>`after`: `nodeCanvasObject` is applied after the default node painting takes place.</li> | `replace` |
